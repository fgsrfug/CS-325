/*******************************************
 * Jesus Narvaez
 * CS 325 Homework 1
 * mergeTime.cpp
 * January 15th, 2020
 * This file uses merge sort to sort a random
 * array of integers, ranging from 1 to 10000
 * of size n and times it.
*******************************************/

#include <iostream>
#include <fstream>
#include <stdio.h>
#include <ctime>
#include <stdlib.h>

using namespace std;

/*******************************************
* Function name: print_array
* Description: Given an array and the size of
*   the array, the elements of the array are
*   printed to the terminal. This function was
*   used mainly in debugging.
*******************************************/
void print_array(int dataset[], int size){
    //Iterate through for the size of the array
    for (int i = 0; i < size; i++)
        cout << dataset[i] << " ";
    cout << endl;
    return;
}

/*******************************************
* Function name: print_to_file
* Description: Given an array and the size of
*   the array, it uses the output stream class
*   to print the elements of the array into a file
*   called "merge.txt".
* This function is not used in this program
*******************************************/
void print_to_file(int dataset[], int size){

    //Open the file to output
    ofstream out_file;
    out_file.open ("merge.txt");

    //Iterate through the array and print each element to our file
    for (int i = 1; i < size + 1; i++)
        out_file << dataset[i] << " ";
    out_file << endl;

    //Close the file
    out_file.close();
    return;
}


/*******************************************
* Function name: merge
* Description: Given an array, the index to
*   the start, end and mid point of the
*   function, this algorithm takes two
*   subarrays and orders them in the dataset
*   array.
* This function was modeled off an example
* given by GeeksforGeeks, with some changes and additions.
*   Title: Merge Sort
*   Author: GeekforGeeks
*   Date: January 14th, 2020
    Availability: https://www.geeksforgeeks.org/merge-sort/
*******************************************/
void merge(int dataset[], int l_index, int r_index, int midpoint){
    //Determine the size of temp arrays to create using indexes
    //that were passed in
    int l_array_size = midpoint - l_index + 1;
    int r_array_size = r_index - midpoint;

    //Create temp arrays and initialize them to 0
    int l_array[l_array_size];
    for (int k = 0; k < l_array_size; k++)
        l_array[k] = 0;

    int r_array[r_array_size];
    for (int k = 0; k < r_array_size; k++)
        r_array[k] = 0;

    //Fill the temp arrays with actual values from dataset
    for (int i = 0; i < l_array_size; i++)
        l_array[i] = dataset[i + l_index];
    for (int j = 0; j < r_array_size; j++){
        r_array[j] = dataset[j + midpoint + 1];
    }

    //Create new indices to to iterate temp arrays and dataset array
    int left = 0;
    int right = 0;
    int main = l_index;

    //While there is an element in both arrays, keep comparing them
    //Which ever element is smaller between the two arrays get placed
    //into the dataset array.
    while ((left < l_array_size) && (right < r_array_size)){
        if (l_array[left] <= r_array[right]){
            dataset[main] = l_array[left];
            left++;
        }
        else {
            dataset[main] = r_array[right];
            right++;
        }
        main++;
    }

    //Make sure that any leftovers in either array gets placed last
    //in dataset.
    while (left < l_array_size){
        dataset[main] = l_array[left];
        left++;
        main++;
    }
    while (right < r_array_size){
        dataset[main] = r_array[right];
        right++;
        main++;
    }
}

/*******************************************
* Function name: merge_sort
* Description: Given an array and the indices
*   to the leftmost and rightmost spots in the
*   array, it determines a midpoint and continues
*   to call itself until there is nothing left
*   to split. Each go through, it also calls
*   merge() to take the two subarrays created
*   and put them together.
* This function was modeled off an example
* given by GeeksforGeeks, with some changesi and additions.
*   Title: Merge Sort
*   Author: GeekforGeeks
*   Date: January 14th, 2020
    Availability: https://www.geeksforgeeks.org/merge-sort/
*******************************************/
void merge_sort(int dataset[], int l_index, int r_index){
    if (l_index < r_index){
        int midpoint = (l_index + r_index)/2;
        merge_sort(dataset, l_index, midpoint);
        merge_sort(dataset, midpoint + 1, r_index);
        merge(dataset, l_index, r_index, midpoint);
    }
}

/*******************************************
*Function name: random_array
* Description: Takes an array of size "size"
* *   and fills each index with a random number
* *   from 1 to 10000
*******************************************/
void random_array(int dataset[], int size){
        for (int i = 0; i < size; i++)
                    dataset[i] = rand() % 10001;
            return;
}

/*******************************************
* Function name: main
* Description: Creates an array of size n,
*   fills it with random elements and sorted
*   using merge sort. The time elapsed running
*   the algorithm is reported at the end.
*******************************************/
int main(){
    //Seed random number generator and init start_time
    srand(time(NULL));
    clock_t start_time;

    //Set the size of the array here!
    int n = 900000;
    int data[n];

    //Fill the array with random numbers
    random_array(data, n);

    //Set indices for use in merge sort
    int l_index = 0;
    int r_index = n - 1;

    //Time merge sort
    start_time = clock();
    merge_sort(data, l_index, r_index);
    start_time = clock() - start_time;

    //Divide CPU ticks by CLOCKS_PER_SEC and present results
    double total_time = ((float) (start_time)) / CLOCKS_PER_SEC;
    cout << "Array size: " << n << endl;
    cout << "Total time merging: " << total_time << " seconds" <<  endl;

    return 0;
}
